...
public class OMEDownload {
  ...
  private void download(Image image, PixelsFactory pf, OMESidePanel omesp, 
	...
	"GreenChannelOn", "RedChannelOn"};
      Criteria criteria = makeAttributeFields(attrs);
      ...
      //red
      criteria = makeAttributeFields(attrsC);
      criteria.addWantedField("id");
...
//    }
  ...
  public void run(OMESidePanel osp) {
	...
	download(images[i], pf, osp, df);
      ...
      OMELoginPanel.infoShow(IJ.getInstance(), 
        "Download Completed Successfully.", "OME Download");
    }
    catch (Exception exc) {
      OMEDownPanel.error(IJ.getInstance(), "An error occured.\n" + 
        exc.toString(), "Error");
