...
public class OMEXML200809Metadata extends OMEXMLMetadata {
  ...
  public int getROICount(int imageIndex) {
    // NB: ROI unsupported for schema version 2008-09
    return -1;
  }
  ...
  private UUIDNode uuidNode = null;
  private int uuidNodeImageIndex = -1;
  private int uuidNodePixelsIndex = -1;
  private int uuidNodeTiffDataIndex = -1;
  private UUIDNode getUUIDNode(int imageIndex, int pixelsIndex, int tiffDataIndex, boolean create) {
    // check whether indices match last request (i.e., node is cached)
    boolean match = true;
    if (uuidNodeImageIndex != imageIndex) match = false;
    if (uuidNodePixelsIndex != pixelsIndex) match = false;
    if (uuidNodeTiffDataIndex != tiffDataIndex) match = false;
    if (match) return uuidNode;
    uuidNode = null;
    uuidNodeImageIndex = imageIndex;
    uuidNodePixelsIndex = pixelsIndex;
    uuidNodeTiffDataIndex = tiffDataIndex;
