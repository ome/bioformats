...
public class JPEG2000Codec extends BaseCodec
{
  ...
  private static final String NO_J2K_MSG = "The JAI Image I/O Tools are required to read JPEG-2000 files. Please "
  ...
  private static final String J2K_WRITER_PARAM = "com.sun.media.imageio.plugins.jpeg2000.J2KImageWriteParam";
  ...
  public byte[] compress(byte[] data, CodecOptions options)
    ...
    if (options == null)
      j2kOptions = JPEG2000CodecOptions.getDefaultOptions();
    else{
      j2kOptions = JPEG2000CodecOptions.getJ2KOptions(options);
    }
    compress(data, j2kOptions);
    return null;
  }

  /**
   * The JPEG2000CodecOptions parameter should have the following fields set:
   * {@link CodecOptions#width width} {@link CodecOptions#height height}
   * {@link CodecOptions#bitsPerSample bitsPerSample}
   * {@link CodecOptions#channels channels} {@link CodecOptions#interleaved interleaved} {@link CodecOptions#littleEndian littleEndian}
   * {@link CodecOptions#lossless lossless}
   * 
   * @see Codec#compress(byte[], CodecOptions)
   */
  public byte[] compress(byte[] data, JPEG2000CodecOptions options)
      throws FormatException {
    if (options == null)
    ...
    try {
      ...
      r.setVar("lossless", options.lossless);
