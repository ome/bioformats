    ...
    implements BATManaged, BlockWritable, POIFSViewable
{
    ...
    public POIFSDocument(final String name, final RawDataBlock [] blocks,
        ...
        throws IOException
    {
        bigBlockSize = blocks[0].getBlockSize();
        _size        = length;
      ...
      int size) throws IOException
    {
        ...
        while (true)
        {
            DocumentBlock block     = new DocumentBlock(stream, size);
    ...
    public POIFSDocument(final String name, final int size,
                         ...
                         final POIFSWriterListener writer, int blockSize)
        throws IOException
    {
    ...
    void read(final byte [] buffer, final int offset, RandomAccessInputStream stream)
    {
        ...
        {
            if (bigBlockSize == 0) bigBlockSize = _big_store.getBigBlockSize();
    ...
    private class SmallBlockStore
    {
        ...
        int getBigBlockSize() { return smallBlocks[0].getBigBlockSize(); }
    ...
    private class BigBlockStore
    {
            ...
            for (int j = 0; j < blocks.length; j++)
            {
                ...
                else
                {
                    bigBlocks[ j ] =
                        ...
                        ((RawDataBlock) blocks[j]).getBlockSize());
                }
            }
            this.path   = null;
            this.name   = null;
        ...
        int getBigBlockSize() { return bigBlocks[0].getBigBlockSize(); }
