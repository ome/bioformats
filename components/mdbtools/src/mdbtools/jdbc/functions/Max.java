...
public class Max implements FunctionExecutor
{
  // max is for either an int or a string
    ...
    throws SQLException
  {
    if (function.getArgumentCount() != 1)
      throw new SQLException("max only works on a single column");
    ...
    for (int i = 0;i < table.num_cols;i++)
    {
      ...
      if (columnName.equalsIgnoreCase(col.name))
      {
        ...
        if (col.col_type == Constants.MDB_INT)
          ...
          throw new SQLException("max only works on integers and strings");
    ...
    throws SQLException
  {
    if (isInteger)
    {
      ...
      throw new SQLException("only max(integer) is implemented");
  }
